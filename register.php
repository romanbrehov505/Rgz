...
if(isset($_POST['verify'])){
	...
	if(!$user){
		...
		$mysql = new Mysql();
		$mysql->connect();
		if ($mysql->insertUser($ISuser, $ISpassword)) {
			echo("<script>alert('Registro efetuado com sucesso!')</script>");
			echo("<script>location.href = 'index.php'</script>");	
		}else{
			echo("<script>alert('Trouble with connicting to DB. Try later!')</script>");
		}
	}else{
	...
	<main id="container">
		...
		<section id="register-box">
			...
			<header class="register-title">
				<h2 style="margin-bottom:0px; margin-top: 25px; font-size: 25pt;">Регистрация</h2>
			</header>
